...
**问题**：
- 当前实现：每次请求都遍历所有 `tag_matching_rules` 进行匹配
- 当规则达到 1 万条时，性能会显著下降（O(n) 复杂度）
- 没有预建的倒排索引，匹配效率低

**当前实现**：
```python
# backend/app/prompts/tag_based_rag.py
# 每次都要遍历所有规则
...
**优化方案**：
- 创建 `tag_inverted_index` 表，预建倒排索引
- 从 `match_pattern` 提取 tokens（例如 "environmental fee" → ["environmental", "fee"]）
- 建立 `token → tag_id` 的映射关系
- 匹配时直接查询索引表，而不是遍历所有规则

**预期效果**：
- 匹配性能从 O(n) 降低到 O(1) 或 O(log n)
- 支持 1 万+ 规则时仍保持良好性能
- 为扩展到 10 万+ 规则做准备

**实现位置**：
- 数据库：`backend/database/XXX_add_tag_inverted_index.sql`
- 代码：`backend/app/prompts/tag_based_rag.py` - 修改 `detect_tags_from_ocr()`

**优先级**：中等（当前设计可支持 1 万条规则，但需要优化以支持更大规模）
...
**当前设计**：
- `prompt_tags.tag_name` = category name（例如 "environmental_fee"）
- `prompt_tags.tag_type` = category type（例如 "validation_rule"）
- 一个 tag 可以有多个 snippets（通过 `tag_id` 关联）
- 多个匹配规则可以触发同一个 tag（多对多关系）

**设计说明**：
- **Tag = Category**：`prompt_tags` 表实际上就是 category 表
- **层级关系**：当前设计是扁平的，没有显式的层级关系
  - 如果需要层级（例如 "fee" → "environmental_fee", "bottle_deposit"），可以通过 `tag_type` 或命名约定实现
  - 或者未来添加 `parent_tag_id` 字段支持树形结构

**当前架构对齐 GPT 建议**：
- ✅ Token → Categories：`tag_matching_rules` → `prompt_tags`（已实现）
- ✅ Categories → Snippets：`prompt_tags` → `prompt_snippets`（已实现）
- ✅ 多对多关系：支持（已实现）
- ⚠️ 倒排索引：实时计算，未预建（需要优化）

**未来扩展**：
- 如果需要层级分类，可以添加 `parent_tag_id` 字段
- 如果需要更复杂的分类体系，可以考虑添加 `tag_category` 表

---

#### 3. Redis 缓存优化 ⚠️ 性能优化（未来）

**问题**：
- 当前实现：简单的内存缓存（`_tag_cache`, `snippet_cache`）
- 缓存不会自动更新（需要手动 `clear_cache()`）
- 多实例部署时缓存不同步
- 没有缓存失效策略
...
_snippet_cache: Dict[str, List[Dict[str, Any]]] = {}
_matching_rules_cache: List[Dict[str, Any]] = []
```

**优化方案**：
- 使用 Redis 作为分布式缓存
- 实现缓存 TTL（Time To Live）
- 监听数据库变更，自动刷新缓存（或使用数据库通知机制）
- 支持多实例部署时的缓存同步
...
**实现位置**：
- 新增：`backend/app/services/cache/redis_client.py`
- 修改：`backend/app/prompts/tag_based_rag.py` - 使用 Redis 缓存

**优先级**：低（当前内存缓存可满足需求，但多实例部署时需要）

**预计工作量**：2-3 天

**依赖**：
- Redis 服务器
- Python Redis 客户端库（`redis` 或 `hiredis`）

---

**总结**：
- 当前 RAG 系统设计已基本对齐 GPT 建议的架构（95% 对齐度）
- 核心功能完整，可以支持 1 万条规则
- 主要优化点：倒排索引（性能）、Redis 缓存（扩展性）
- 这些优化可以在规则数量增长到瓶颈时再实施
