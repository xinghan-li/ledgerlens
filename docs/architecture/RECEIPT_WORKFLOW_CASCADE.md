...
### 4.1 store_candidates 写入（文本流程与 Vision 流程一致）

两种流程都会在「无匹配 store 或新 location」时把 **potential candidate** 写进 `store_candidates`，供 Admin Store Review 审核后创建 chain/location。逻辑统一：`get_store_chain` 未匹配则创建一条 candidate（含 `suggested_chain_id` / `suggested_location_id` / `confidence_score` 等）。

```mermaid
flowchart LR
    subgraph Text["文本流程 (legacy_a)"]
        T1[LLM 首轮/回退] --> T2[receipt_llm_processor]
        T2 --> T3{get_store_chain 匹配?}
        T3 -->|否| T4[create_store_candidate]
        T3 -->|是| T5[不创建 candidate]
    end
    subgraph Vision["Vision 流程 (vision_b)"]
        V1[vision_primary / escalation] --> V2[categorize_receipt]
        V2 --> V3[save_receipt_summary]
        V3 --> V4{store_chain_id 有?}
        V4 -->|否| V5[create_store_candidate<br/>步骤 5b]
        V4 -->|是| V6[不创建 candidate]
    end
    T4 --> DB[(store_candidates)]
    V5 --> DB
```

| 流程 | 写入时机 | 代码位置 |
|------|----------|----------|
| **文本** | LLM 阶段结束后，无匹配或仅 chain 匹配无 location | `receipt_llm_processor`：`need_store_candidate` → `create_store_candidate(..., suggested_chain_id, suggested_location_id, confidence_score)` |
| **文本** | Vision OCR+RBSJ 回退路径，无匹配时 | `workflow_processor`：vision 回退后 `get_store_chain` 未匹配 → `create_store_candidate` |
| **Vision** | 无论 Sum Check 通过或 needs_review，都会调用 `categorize_receipt`；在 categorizer 内无 store 匹配时 | `receipt_categorizer` 步骤 5b：`effective_chain_id` 为空且已有店名 → `get_store_chain` 取 suggested_* → `create_store_candidate(..., suggested_chain_id, suggested_location_id, confidence_score)` |

因此 **Vision 流程与文本流程在「把 potential candidate 写进数据库」这一点上一致**，均通过统一的 `create_store_candidate` 入参（含 `llm_result.receipt`、suggested_*、confidence_score）写入 `store_candidates`。
...
- **落库**：items → classification_review；无匹配 store 或新 location → store_candidates（**文本与 Vision 流程均通过 categorize_receipt / receipt_llm_processor 等同一套逻辑写入**）；各阶段 → receipt_processing_runs，失败仅写 error + reason。
...
| **store_candidates（文本）** | `receipt_llm_processor`：LLM 结束后无匹配/新 location → `create_store_candidate`；`workflow_processor`：vision 回退路径无匹配 → `create_store_candidate` |
| **store_candidates（Vision 流程）** | `workflow_processor_vision` 成功/needs_review 均调 `categorize_receipt` → `receipt_categorizer` 步骤 5b 无 store 匹配时 `create_store_candidate`（与文本流程同一套入参：suggested_*、confidence_score） |
