-- ============================================
-- Migration 017: Link receipt_items to products catalog
-- ============================================
-- Purpose: Add product_id foreign key to receipt_items to enable
...
-- ============================================
-- 1. Add product_id column to receipt_items
-- ============================================
...
CREATE INDEX receipt_items_category_id_idx ON receipt_items(category_id);

-- Composite indexes for common queries
CREATE INDEX receipt_items_user_product_idx ON receipt_items(user_id, product_id);
CREATE INDEX receipt_items_product_date_idx ON receipt_items(product_id, created_at DESC);
CREATE INDEX receipt_items_user_category_idx ON receipt_items(user_id, category_id);

-- ============================================
-- 4. Add comments
-- ============================================
COMMENT ON COLUMN receipt_items.product_id IS 'Foreign key to products table for normalized product aggregation';
COMMENT ON COLUMN receipt_items.category_id IS 'Direct category link (can override product.category_id if needed)';
...
SELECT 
  ...
  -- Store info (from receipt_summaries)
  rs.store_chain_id,
...
BEGIN
  SELECT COUNT(*) INTO items_count FROM receipt_items;
  SELECT COUNT(*) INTO items_with_product FROM receipt_items WHERE product_id IS NOT NULL;
  
  RAISE NOTICE 'Migration 017 completed successfully.';
  RAISE NOTICE 'Total receipt_items: %', items_count;
  ...
  RAISE NOTICE '2. Backfill existing receipt_items with product_id';
...
-- ============================================
-- Once all receipt_items have product_id, run:
-- ALTER TABLE receipt_items ALTER COLUMN product_id SET NOT NULL;
