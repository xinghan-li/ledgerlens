"""
DEPRECATED — Legacy OCR + LLM receipt processing workflow.

This module is no longer used. The active pipeline is vision-only (see app.core.workflow_processor_vision).
Kept in backend/deprecated/ for reference only. Not imported by the application.

Legacy workflow:
1. Google Document AI OCR
2. Gemini or GPT-4o-mini LLM from OCR text
3. Sum check; on failure AWS Textract + secondary LLM
4. File storage and timeline recording
"""
...
from app.core.workflow_common import (
    PROJECT_ROOT,
    OUTPUT_ROOT,
    INPUT_ROOT,
    TimelineRecorder,
...
from app.services.ocr.documentai_client import parse_receipt_documentai
from app.services.ocr.textract_client import parse_receipt_textract
from app.services.llm.gemini_rate_limiter import check_gemini_available, record_gemini_request
from app.services.llm.receipt_llm_processor import process_receipt_with_llm_from_ocr
from app.processors.core.sum_checker import check_receipt_sums, apply_field_conflicts_resolution, _effective_tolerance
from app.services.llm.llm_client import parse_receipt_with_llm, parse_receipt_with_openai_vision
from app.prompts.prompt_manager import format_prompt, get_default_prompt
from app.prompts.prompt_loader import get_debug_prompt_system
...
from app.services.database.statistics_manager import record_api_call
...
from app.processors.enrichment.address_matcher import match_store, fix_ocr_address, correct_address
from app.services.ocr.ocr_normalizer import normalize_ocr_result, extract_unified_info
from app.exporters.csv_exporter import convert_receipt_to_csv_rows, append_to_daily_csv, get_csv_headers
from app.processors.stores.chain_cleaners import apply_chain_cleaner
from app.processors.text.data_cleaner import clean_llm_result
...
# Legacy escalation prompt (image-based fallback)
